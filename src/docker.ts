...
export async function listContainers(): Promise<ContainerSummary[]> {
	...
	return raw.map((c) => ({
		id: SHORT(c.Id),
		name: (c.Names?.[0] ? c.Names[0] : c.Id).replace(/^\//, ""),
		image: c.Image,
		state: c.State,
		status: c.Status,
	}));
...
export async function logs(id: string, tail = 100): Promise<LogsResult> {
	...
	let text = demux(
		Buffer.isBuffer(buf) ? buf : Buffer.from(buf as unknown as ArrayBuffer),
		meta.tty,
	);
	// Strip ANSI escape codes for clean display in Telegram. The ESC byte (0x1B)
	// is built at runtime so the regex source carries no control-char literal.
	const ESC = String.fromCharCode(27);
	text = text.replace(new RegExp(`${ESC}\\[[0-9;]*[A-Za-z]`, "g"), "");
	return { name: meta.name, text };
}

// Like logs(), but only returns lines emitted at/after `since` (unix seconds).
// Used by the log monitor to test just the NEW output each interval instead of
// re-scanning the whole tail. `since` is second-granular, so a line landing
// exactly on the boundary can occasionally appear in two adjacent polls — the
// monitor tolerates that rather than risk skipping logs within the same second.
export async function logsSince(id: string, since: number): Promise<string> {
	const meta = await inspect(id);
	const buf = await docker.getContainer(id).logs({
		stdout: true,
		stderr: true,
...
export async function stats(id: string): Promise<StatsResult> {
	...
	const cpuDelta =
		s2.cpu_stats.cpu_usage.total_usage - s1.cpu_stats.cpu_usage.total_usage;
	const sysDelta =
		s2.cpu_stats.system_cpu_usage - s1.cpu_stats.system_cpu_usage;
	const cpus =
		s2.cpu_stats.online_cpus ||
		(s2.cpu_stats.cpu_usage.percpu_usage
			? s2.cpu_stats.cpu_usage.percpu_usage.length
			: 1);
	const cpuPct =
		sysDelta > 0 && cpuDelta > 0 ? (cpuDelta / sysDelta) * cpus * 100 : 0;

	const inactive = s2.memory_stats.stats?.inactive_file || 0;
